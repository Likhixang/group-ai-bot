...
- `/av 番号`（例如 `/av ABP-001`）：通过 R18.dev 查询封面；到全局清理 TTL 后删除命令与封面结果
- 回复一张图片后发送 `/av`，或发送图片时在 caption 写 `/av`：通过 AVScan 检索番号；到全局清理 TTL 后删除源图、命令与结果
- `/new` 清空你在当前聊天的上下文记忆
...
- 同时轮询 Codex Reset RSS 和 Codex Resets API；任一来源先确认就播报，两个来源在 6 小时内确认同一事件时自动去重
- `/reset` 命令查询最近一次确认重置和下一次预计重置，并附对应来源链接；命令和回复 30 秒后自动删除
- 确认重置通知临时置顶 30 分钟；首次轮询只建立历史基线，不补发旧事件
