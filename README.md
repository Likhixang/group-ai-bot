...
- `GROK_API_BASE_URL` — `/gkimg` and `/vid` API base URL
- `GROK_API_KEY` — dedicated API key for `/gkimg` and `/vid`
...
- `/vid your prompt` — generate a video with `grok-imagine-video-1.5`
...
- `/av <code>` (for example, `/av ABP-001`) — retrieve the jacket from R18.dev; after the global cleanup TTL, the command and cover result are deleted
...
- Poll both the Codex Reset RSS feed and the Codex Resets API; announce the first confirmed observation and deduplicate matching cross-source events within six hours
...
- Keep `AI_API_KEY`, `GROK_API_KEY`, and `TELEGRAM_BOT_TOKEN` only in environment variables.
