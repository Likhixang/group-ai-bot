...
def _is_text_ai_prefix(raw_text: str) -> bool:
    ...
    return (
        ...
        or text.startswith("/ln ")
        or text.startswith("lm ")
        or text.startswith("/lm ")
        or text in {"ds", "/ds", "gk", "/gk", "gm", "/gm", "ln", "/ln", "lm", "/lm"}
    )
...
def _clean_prompt(raw_text: str) -> str:
    text = raw_text.strip()
    low = text.lower()
    for prefix in ("/gk ", "gk ", "/ds ", "ds ", "/gm ", "gm ", "/ln ", "ln ", "/lm ", "lm "):
        if low.startswith(prefix):
            text = text[len(prefix):].strip()
            break
    else:
        if low in {"/gk", "gk", "/ds", "ds", "/gm", "gm", "/ln", "ln", "/lm", "lm"}:
            text = ""
...
def _inherit_text_model_from_reply(msg) -> Optional[str]:
    ...
    if not model_name:
        ...
        return None
    if model_name in {DS_MODEL, AI_THINKING_MODEL, OAI_MODEL, GROK_MODEL, GEMINI_MODEL, LUNA_MODEL, LUMO_MODEL}:
...
def _select_text_model(raw_text: str, msg=None) -> str:
    ...
    if low.startswith("/ln ") or low.startswith("ln ") or low in {"/ln", "ln"}:
        return LUNA_MODEL
    if low.startswith("/lm ") or low.startswith("lm ") or low in {"/lm", "lm"}:
        return LUMO_MODEL
...
async def ai_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /ds, /gk, /gm and /lm commands by forwarding to on_text with prefix restored."""
    msg = update.effective_message
    if not msg:
        return
    ...
    if not (is_generation or is_edit or reply_image_file_id or (own_image_file_id and is_edit)):
        ...
        if (
            (
                ...
                or raw_lower.startswith("lm ")
                or raw_lower.startswith("ln ")
...
async def _review_link_content_with_luna(url: str) -> str:
    ...
    prompt = (
        f"群里有人丢了个链接过来，你瞅一眼，直接用群友聊天的口吻给大伙锐评一下：\n\n"
        f"链接地址: {url}\n"
        f"抓到的内容:\n{prompt_content}\n\n"
        f"注意：\n"
        f"1. 说话千万别像个AI总结机！别整什么“本文介绍了”、“综上所述”这种机器人官话。直接口语化吐槽或者说人话，一针见血说清楚这玩意到底在逼逼啥、有没有点有用的东西、值不值得点进去看。\n"
        f"2. 评完之后，另起一行给出你对这篇内容的「含屎量」打分（0-100%），格式固定为：\n"
        f"含屎量：X%\n"
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "你叫 Luna，是 Telegram 群里一个见多识广、说话尖锐刻薄又幽默的资深群友（老老司机）。\n"
                "你的特点：\n"
                "1. 极其反感互联网上的营销号、割韭菜软文、AI废话通稿、标题党和洗稿垃圾。\n"
                "2. 说话必须是地道的真人聊天口吻，口语化、短句、带点网络调侃，绝对不要像公文或无情的摘要机器人。严禁使用“本文主要讲述了”、“作者从几个方面进行了阐述”等死板AI句式。\n"
                "3. 评析逻辑：三两句话扒掉作者底裤，直接说透这玩意到底在讲什么、核心有没有价值、是干货还是纯纯的骗点击/注水猪肉。\n"
                "4. 最后一行必须单独输出含屎量：含屎量：X%（X为0-100的数值）。"
            ),
        },
